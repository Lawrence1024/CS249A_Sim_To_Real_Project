...
behavior DriveStraight():
    while True:
        take applyWheeledController(0.10, 0)

behavior DrivePhases():
    for i in range(100):
        take applyWheeledController(0.10, 0)
    for i in range(100):
        take applyWheeledController(0, 1.0)
    for i in range(100):
        take applyWheeledController(0.10, 0)
    while True:
        wait
behavior PulseForward():
    for i in range(60):
        wait
    for i in range(120):
        take applyWheeledController(0.10, 0)
...
ego = new Pololu3Pi at (0, 0, 0.03),
    facing (0 deg, 0 deg, 0),
    with behavior PulseForward